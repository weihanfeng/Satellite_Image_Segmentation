...
class UNetWithResnet50Encoder(nn.Module):
    ...
    def __init__(self, last_n_layers_to_unfreeze, n_classes=5):
        super().__init__()
        resnet = torchvision.models.resnet.resnet50(pretrained=True)
        # freeze all but the last n layers
        total_num_layer = len(list(resnet.children())[:-2])
        for i, block in enumerate(list(resnet.children())):
            if total_num_layer - i > last_n_layers_to_unfreeze:
                for param in block.parameters():
                    param.requires_grad = False
        down_blocks = []
        up_blocks = []
        self.input_block = nn.Sequential(*list(resnet.children()))[:3] # first 3 layers (non-sequential blocks)
        self.input_pool = list(resnet.children())[3] # pooling layer
        for bottleneck in list(resnet.children()): # sequential resnet blocks
