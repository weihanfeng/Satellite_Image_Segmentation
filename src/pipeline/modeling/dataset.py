...
class Transform:
    ...
    def __init__(
        self, zoom_range=(1, 2), brightness_range=0.2, contrast_range=0.2, prob=0.5
    ):
    ...
    def __call__(self, image, mask):
        ...
        if torch.rand(1) < self.prob:
            ...
            zoom_factor = (
                torch.rand(1) * (self.zoom_range[1] - self.zoom_range[0])
                + self.zoom_range[0]
            )
            image = T.functional.affine(
                image, scale=zoom_factor, angle=0, translate=(0, 0), shear=0
            )
            mask = T.functional.affine(
                mask, scale=zoom_factor, angle=0, translate=(0, 0), shear=0
            )
            # random contrast and brightness adjustment
            image = T.ColorJitter(
                brightness=self.brightness_range, contrast=self.contrast_range
            )(image)
